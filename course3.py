...
class question1(Resource):
    def get(self):
        return ("Well you know how to create a basic api. Now lets retreive and return json shall we. Create an api that jsonifies the awesomeDictionaryToReturn to json and return it. USE THE IMPORTED JSON LIBRARY!")


@api.route('/question2')
class question2(Resource):
    def get(self):
        return ("Now we got that working, why dont you retreive json from the request body and add it to the empty parameter in the dict, before returning it")

@api.route('/question3')
class question3(Resource):
    def get(self):
        return ("EXPERTS! Same as question 3 now have a look at api.models and @api.expect and make sure the request api is documented in swagger")
